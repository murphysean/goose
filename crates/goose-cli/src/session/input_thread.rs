...
pub struct Printer {
    inner: Option<std::sync::Mutex<Box<dyn ExternalPrinter + Send>>>,
}

impl Printer {
    /// Print a message above the prompt without disturbing user input.
    pub fn print(&self, msg: String) {
        if let Some(printer) = &self.inner {
            let _ = printer.lock().unwrap().print(msg);
        }
    }
}
...
pub fn spawn_input_thread(
    ...
    // If we're not in a TTY (e.g. piped input), the printer is a no-op.
    ...
    InputHandle {
        ...
        printer: Printer {
            inner: printer
                .map(|p| std::sync::Mutex::new(Box::new(p) as Box<dyn ExternalPrinter + Send>)),
        },
        interrupt_state,
