...
impl CliSession {
    ...
    async fn run_interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        self.display_context_usage(&input_handle.printer).await?;
        ...
        loop {
            ...
            tokio::select! {
                ...
                Some(task_event) = async {
                    ...
                    // Drain ALL pending task events and batch them into one notification
                    let mut events = vec![task_event];
                    if let Some(rx) = &mut task_event_rx {
                    ...
                    if events.len() == 1 {
                        let prompt = Self::synthesize_task_prompt(&events[0]);
                        input_handle.printer.print(format!(
                            "⚡ {}\n", Self::task_event_summary(&events[0])
                        ));
                        turn_queue.push_system(prompt).await;
                    } else {
                        let summaries: Vec<String> = events.iter().map(Self::task_event_summary).collect();
                        let details: Vec<String> = events.iter().map(Self::synthesize_task_prompt).collect();
                        input_handle.printer.print(format!(
                            "⚡ {} background tasks completed\n", events.len()
                        ));
                        let prompt = format!(
                            "System: {} background tasks have completed:\n{}\n\n{}",
                            events.len(),
            ...
            goose::tasks::TaskEvent::TaskCompleted { task } => {
                ...
                let follow_up = match task.source {
                    goose::tasks::TaskSource::Process => {
                        format!(
                            "Use read_output(task_id: \"{}\") to see the output.",
                            task.id
                        )
                    }
                    ...
                    match result {
                        Some(Ok(AgentEvent::Message(message))) => {
                            ...
                            if let Some((id, security_prompt)) = find_tool_confirmation(&message) {
                                ...
                                let is_thinking_notification = message.content.iter().any(|c| {
                                    matches!(
                                        c,
                                ...
                                if interactive && !is_thinking_notification {
                                    output::hide_thinking();
                                }
                                if !is_thinking_notification {
                                    let _ = progress_bars.hide();
                                }
    ...
    pub async fn display_context_usage(&self, printer: &input_thread::Printer) -> Result<()> {
        let provider = self.agent.provider().await?;
        ...
        match self.get_session().await {
            Ok(metadata) => {
                ...
                printer.print(output::context_usage_line(total_tokens, context_limit));

                if show_cost {
                    if let Some(line) = output::cost_usage_line(
                        &provider_name,
            ...
            Err(_) => {
                printer.print(output::context_usage_line(0, context_limit));
            }
        }
        ...
        ServerNotification::ResourceUpdatedNotification(r) => {
            if is_stream_json_mode {
                emit_stream_event(&StreamEvent::Notification {
                    extension_id: extension_id.to_string(),
                    data: NotificationData::Log {
                        message: format!("Resource updated: {}", r.params.uri),
                    },
                });
            } else if !is_json_mode {
                if interactive {
                ...
                println!("  {} resource updated: {}", extension_id, r.params.uri);
